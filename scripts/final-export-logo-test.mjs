...
const [helper, branding, report, logoAsset] = await Promise.all([
...
assert.match(branding, /EXPORT_LOGO_PNG_BASE64/, 'PDF-Fallback verwendet nicht das transparente Standardlogo.')
assert.match(branding, /readPdfLogoBytes/, 'Zentrale PDF-Logoquelle fehlt.')
assert.match(helper, /readPdfLogoBytes/, 'PDF lädt das zentral gespeicherte Firmenlogo nicht.')
assert.match(helper, /drawCenteredPdfWatermark/, 'PDF zeichnet das Logo nicht als zentriertes Wasserzeichen.')
