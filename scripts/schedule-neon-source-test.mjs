...
assert.match(repository, /rebindInactiveScheduleShifts/)
assert.match(repository, /schedule_employees stale/)
assert.match(repository, /COUNT\(\*\) = 1/)
...
assert.match(schedule, /async function syncActiveEmployees\(\)/)
assert.match(schedule, /hasOtherSameNameEntry/)
assert.match(schedule, /await syncActiveEmployees\(\)/)
const handlerStart = schedule.indexOf('export default async function scheduleV2Neon')
const routeStart = schedule.indexOf('\n  const url = new URL(request.url)', handlerStart)
assert.ok(handlerStart >= 0 && routeStart > handlerStart, 'Dienstplan-Handler konnte nicht eingegrenzt werden.')
assert.doesNotMatch(schedule.slice(handlerStart, routeStart), /syncActiveEmployees\(\)/, 'Normale Dienstplan-Leseaufrufe dürfen nicht auf die Vollsynchronisierung warten.')
