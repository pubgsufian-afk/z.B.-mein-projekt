...
assert.doesNotMatch(refresh, /setInterval/, 'portal refresh must not poll continuously while the app stays open')
assert.doesNotMatch(refresh, /localStorage|sessionStorage|indexedDB|location\.reload/)
assert.match(main, /installDataRefreshTriggers\(\)/, 'main should install event-driven refresh without an interval')
