...
expectIncludes(maintenance, "const DIRECT_TIME_EDIT_ROLES = new Set<Role>(['owner', 'admin', 'manager'])", 'direct editing must allow Hauptadmin, Admin and Einsatzleiter')
expectIncludes(maintenance, "action === 'admin-time-edit'", 'maintenance endpoint must route admin-time-edit')
expectIncludes(maintenance, 'DIRECT_TIME_EDIT_ROLES.has(current.role)', 'server must enforce the three direct-edit roles')
expectIncludes(maintenance, 'attendance_adjustments', 'direct edit must persist an effective pause adjustment for completed sessions')
expectIncludes(maintenance, "'admin-time-edit'", 'direct edit must be written to the audit log')
expectIncludes(maintenance, 'management-clock-out:', 'an open checked-in session must support an administrative clock-out')
expectIncludes(maintenance, 'Bei einem laufenden Dienst kann die Pause erst zusammen mit einem Arbeitsende korrigiert werden.', 'open sessions must not persist an orphan pause adjustment')
