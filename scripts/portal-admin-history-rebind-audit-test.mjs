...
const start = source.indexOf('const auditBefore =')
assert.ok(start >= 0, 'history rebind audit payload not found')
const end = source.indexOf('return { eventCount', start)
assert.ok(end > start, 'history rebind audit block end not found')
const auditSection = source.slice(start, end)
...
assert.match(auditSection, /'admin-employee-rebind'/)
