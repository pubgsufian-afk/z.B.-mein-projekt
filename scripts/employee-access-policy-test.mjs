...
assert.match(app, /\{ key: 'timesheet', label: 'Stundenzettel', roles: \['owner', 'admin', 'manager', 'employee'\] \}/)
for (const key of ['overview', 'worksites', 'reports', 'settings']) {
  assert.doesNotMatch(app, new RegExp(`key: '${key}'[^\n]+employee`), `Mitarbeiter darf ${key} nicht in der Navigation erhalten.`)
}
assert.doesNotMatch(app, /key: 'times', label: 'Zeiten'/)
