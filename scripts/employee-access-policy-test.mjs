...
assert.match(neonSchedule, /let sameNameEmployees = activeEmployees\.filter/)
assert.match(neonSchedule, /sameNameEmployees\.length !== 1/)
assert.match(neonSchedule, /let publishedEntries = await listScheduleShifts\(\{ from, to, publishedOnly: true \}\)/)
assert.match(neonSchedule, /hasOtherSameNameEntry/)
