...
import { verifyScheduleGithubOidc } from '../netlify/functions/_shared/schedule-github-oidc.mts'
...
await assert.rejects(() => verifyScheduleGithubOidc(makeToken({}, { kid: 'unknown-kid' }), now, fakeFetch), /kid|key/i)

const { privateKey: attackerKey } = generateKeyPairSync('rsa', { modulusLength: 2048 })
await assert.rejects(() => verifyScheduleGithubOidc(makeToken({}, {}, attackerKey), now, fakeFetch), /signature/i)
