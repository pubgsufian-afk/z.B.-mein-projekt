...
assert.match(app, /const shiftRequest = apiJson\(`\/api\/schedule-v2\?resource=entries&from=\$\{from\}&to=\$\{to\}`\)/)
assert.match(app, /const auxiliaryRequest = Promise\.all\(\[/)
const shiftRequestIndex = app.indexOf('const shiftRequest = apiJson(`')
