...
assert.match(workflow, /github\.event\.comment\.user\.id\s*==\s*249184348/)
...
assert.match(workflow, /SCHEDULE_PUBLIC_KEY_RESULT_PATH:/)
assert.match(workflow, /github\.event\.comment\.body/)
assert.match(workflow, /Publish public relay key response/)
assert.match(workflow, /habun-schedule-public-key-v1/)
assert.match(workflow, /issues\/73\/comments/)
assert.match(workflow, /RELAY_EXIT_CODE:/)
assert.match(workflow, /netlify-401/)
assert.match(workflow, /netlify-400/)
assert.match(workflow, /Verschlüsselter Dienstplan-Auftrag ist ungültig/)
assert.match(workflow, /classification = 'decrypt'/)
assert.match(workflow, /github-oidc/)
assert.match(workflow, /rejected/)
assert.match(workflow, /habun\/schedule-relay\/\$\{classification\}/)
assert.match(workflow, /repos\/\$\{process\.env\.GITHUB_REPOSITORY\}\/statuses\/\$\{process\.env\.GITHUB_SHA\}/)
...
assert.match(relay, /SCHEDULE_PUBLIC_KEY_RESULT_PATH/)
assert.match(relay, /createDecipheriv/)
assert.match(relay, /publicKeyFromEncryptedResult/)
assert.match(relay, /BEGIN PUBLIC KEY/)
...
assert.doesNotMatch(relay, /console\.log\([^\n]*(oidcToken|envelope|responseBody|employeeName|email|GITHUB_RELAY_TOKEN|encryptedResult|publicKey)/i)
