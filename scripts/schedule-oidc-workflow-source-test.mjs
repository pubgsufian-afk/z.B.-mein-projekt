...
assert.match(workflow, /github\.event\.issue\.number\s*==\s*71/)
assert.match(workflow, /github\.event\.comment\.user\.id\s*==\s*249184348/)
assert.match(workflow, /github\.event\.comment\.body\s*==\s*['"]Dienstplan ausführen['"]/)
assert.match(workflow, /github\.event\.issue\.pull_request\s*==\s*null/)
