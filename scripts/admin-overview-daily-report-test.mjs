...
const pushCssPath = resolve(root, 'frontend/src/push-notifications.css')
...
assert.equal(existsSync(pushCssPath), true, 'push notification styles must exist')
...
const pushCss = readFileSync(pushCssPath, 'utf8')
...
const activatePush = push.match(/async function activatePushNotifications\(\)[\s\S]*?export async function installPushNotifications/)?.[0] || ''
const installPush = push.match(/export async function installPushNotifications\(\)[\s\S]*$/)?.[0] || ''
const validRoleGuardIndex = activatePush.indexOf('ACTIVE_PORTAL_ROLES.has(String(session.role))')
const iosPreflightIndex = activatePush.indexOf('isIOS() && !isStandalone()')
const unsupportedReturnIndex = activatePush.indexOf('if (!pushSupported()) return')
assert.match(push, /function portalReady\(\)/, 'push UI must wait for the authenticated portal shell')
assert.match(installPush, /if \(portalReady\(\)\)/, 'push installation must not render over the login screen')
assert.match(installPush, /MutationObserver/, 'push installation must activate after a successful in-page login')
...
assert.doesNotMatch(push, /Benachrichtigung senden/, 'push controls must not collide with the existing Ende field label')
assert.match(pushCss, /\.habun-push-card\{[^}]*pointer-events:none/, 'push guidance must never block portal controls underneath')
assert.match(pushCss, /\.habun-push-card button\{[^}]*pointer-events:auto/, 'push guidance buttons must remain clickable')
