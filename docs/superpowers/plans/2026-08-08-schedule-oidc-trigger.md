...
- The encrypted command remains version 1, algorithm `RSA-OAEP-256+A256GCM`, and must be no more than 30 minutes old (5-minute future skew allowed).
...
Create `scripts/schedule-github-oidc-test.mjs` with a generated RSA test key. Build an RS256 JWT containing the exact required issuer, audience, repository, ref, subject and workflow ref, then assert the verifier accepts it. Add negative assertions for wrong audience, wrong repository, wrong ref, wrong workflow ref, expired token, future `nbf`, non-RS256 `alg`, unknown `kid`, and invalid signature.
...
Claims validation must be explicit:
...
`script/schedule-oidc-trigger-source-test.mjs` must assert that the function imports the OIDC verifier, verifies before decrypting, reads `SCHEDULE_COMMAND_PRIVATE_KEY_B64` and `SCHEDULE_ASSISTANT_TOKEN` only via `Netlify.env.get`, calls the assistant internally, exposes only `/api/schedule-oidc-trigger`, accepts only POST, contains no CORS header, contains no `database.pool.query`, and does not import attendance/account/role mutation modules.

Also extend envelope tests so the runtime decryptor is tested against the same generated RSA/AES envelope contract as the existing build decryptor.
...
Create `schedule-command-envelope-runtime.mts` using Node `privateDecrypt` with RSA-OAEP SHA-256 and `createDecipheriv('aes-256-gcm', ...)`. Enforce envelope version 1, state `command`, algorithm `RSA-OAEP-256+A256GCM`, 32-byte AES key, 12-byte IV and 16-byte GCM tag. Return the parsed JSON object; never log plaintext.
...
await verifyScheduleGithubOidc(String(body.oidcToken || '')) // first security gate
...
Call `scheduleAssistant` internally with a synthetic POST request and `Authorization: Bearer ${assistantToken}`. For `publish-shifts`, pass `requestId: command.commandId` and at most 100 shifts. Return only a privacy-minimized result:
...
The test must assert the workflow:
...
and that it triggers only on pushes to `main` when `ops/schedule-command.envelope.json` or `ops/schedule-command-trigger.txt` changes. It must not reference `SCHEDULE_ASSISTANT_TOKEN`, `SCHEDULE_ASSISTANT_BRIDGE_TOKEN`, `SCHEDULE_COMMAND_PRIVATE_KEY_B64`, repository secrets, database URLs, or write permissions.
...
Exit nonzero for HTTP errors or `rejectedCount > 0`; do not print response bodies containing unexpected fields.
...
Append these focused tests after existing schedule command tests:
...
Expected: all pass. If the repository's existing double-verify mutation bug appears only on the second `verify` inside `npm run build`, separate that unrelated regression from this feature and fix it only if the latest `main` still contains it; do not weaken OIDC tests.
...
- Modify: `netlify/functions/schedule-assistant.mts` if the current `main` still accepts `SCHEDULE_ASSISTANT_BRIDGE_TOKEN`
...
- Netlify configuration: `SCHEDULE_COMMAND_PRIVATE_KEY_B64`, `SCHEDULE_ASSISTANT_TOKEN`, remove obsolete command/bridge variables after successful rollout
...
Remove `node scripts/process-schedule-command-build.mjs` from the `build` script. Keep the encrypted envelope format and public key; the OIDC workflow now transports the envelope.

- [ ] **Step 2: Remove legacy bridge-token authorization**

Update `scripts/apply-schedule-command-worker.mjs` so it no longer patches `SCHEDULE_ASSISTANT_BRIDGE_TOKEN` into `schedule-assistant.mts`. Ensure `schedule-assistant` accepts only `SCHEDULE_ASSISTANT_TOKEN`. Preserve the `sync-directory` action.

Add/update source tests to assert `SCHEDULE_ASSISTANT_BRIDGE_TOKEN` is absent from the assistant and OIDC workflow.
...
Require a successful Netlify deploy preview and no secret-scan matches before merge.

- [ ] **Step 4: Merge and verify production function**

Merge only the verified current-main PR. Confirm the production deploy is `ready`, branch `main`, and `schedule-oidc-trigger` appears in `available_functions` with route `/api/schedule-oidc-trigger`.

- [ ] **Step 5: Ensure runtime secrets without exposing values**

`SCHEDULE_COMMAND_PRIVATE_KEY_B64` must be available to production Functions/Runtime. `SCHEDULE_ASSISTANT_TOKEN` remains available to production Functions/Runtime. If scope cannot be changed without exposing/re-entering the existing private key, rotate the RSA keypair: store the new private key only as a Netlify secret and commit only the new public key to `ops/schedule-command-public.pem`.

Do not expose secret values in chat, commits, logs, or workflow files.

- [ ] **Step 6: Remove obsolete Netlify triggers after OIDC is live**

Delete `SCHEDULE_ASSISTANT_COMMAND_RUNTIME` and `SCHEDULE_ASSISTANT_BRIDGE_TOKEN` from Netlify after a successful OIDC dry run. The scheduled worker may remain deployed temporarily but must have no command secret; it can then be removed in a later cleanup PR without blocking this rollout.

- [ ] **Step 7: Create a fresh encrypted Saturday command**

Encrypt this exact payload with `ops/schedule-command-public.pem` and a fresh `createdAt`:

```json
{
  "version": 1,
  "commandId": "saturday-2026-08-08-<fresh-unique-suffix>",
...
Commit only the encrypted envelope to `main`. If the envelope content hash is unchanged for any reason, update `ops/schedule-command-trigger.txt` with a non-sensitive unique trigger string.

- [ ] **Step 8: Verify workflow execution**

Confirm the `Habun schedule OIDC relay` run was triggered by the exact command commit. The log may show only numeric counts/statuses; it must not expose OIDC JWTs, plaintext names/times, private keys, assistant tokens, or database credentials.
...
Also read recent `schedule_audit_log` entries for actor `dienstplan-assistent` and action `shift-published`.

Only report each employee as entered if the corresponding live row exists. If Amin or Sarmad are not registered/active, report them as not entered rather than creating guessed employee records. Exact duplicates count as already entered and are not recreated.
