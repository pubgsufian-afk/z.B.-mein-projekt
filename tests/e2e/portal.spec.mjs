...
test('V2 schedule, live view and reports select employees by name', async ({ page }) => {
  ...
  const scheduleSection = dialog.locator('[data-section="schedule"]')
  const employeeSelect = scheduleSection.locator('select[name="employeeUserId"]')
  ...
  await expect(scheduleSection.locator('input[name="employeeName"]')).toHaveValue('Anna Beispiel')

  await dialog.getByRole('button', { name: 'Live-Übersicht', exact: true }).click()
  const liveSection = dialog.locator('[data-section="live"]')
  await expect(liveSection.locator('select[data-live-user]')).toBeVisible()
  await expect(liveSection.locator('select[data-live-user]')).toContainText('Anna Beispiel')
  await expect(liveSection.locator('select[data-live-object]')).toContainText('Objekt Nord')

  await dialog.getByRole('button', { name: 'Berichte', exact: true }).click()
  const reportsSection = dialog.locator('[data-section="reports"]')
  const reportEmployees = reportsSection.locator('select[name="employeeSelection"]')
  await expect(reportEmployees).toBeVisible()
