...
function encryptResult(data: unknown, encodedKey: string) {
...
type ReconcileCommand = {
  ...
  to: string
  responseKey: string
  protectedEmployees: Array<{ canonicalName: string; aliases: string[] }>
  shifts: Array<{ employeeName: string; date: string; start: string; end: string; workArea: string; pauseMinutes: number }>
}

function validateCommand(value: unknown, now = new Date()): ReconcileCommand {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('invalid command')
  const row = value as Record<string, unknown>
  if (row.version !== 1 || text(row.action) !== 'reconcile-range') throw new Error('invalid action')
  const commandId = text(row.commandId, 160)
  ...
  const from = text(row.from, 10)
  const to = text(row.to, 10)
  ...
  if (!validResponseKey(row.responseKey)) throw new Error('invalid response key')

  const protectedEmployees = Array.isArray(row.protectedEmployees) ? row.protectedEmployees : []
  ...
  const protectedRows = protectedEmployees.map((item) => {
    ...
    const canonicalName = text(input.canonicalName, 160)
    const aliases = Array.isArray(input.aliases) ? input.aliases.map((alias) => text(alias, 160)).filter(Boolean) : []
  ...
  const cleanedShifts = shifts.map((item) => {
    ...
    const input = item as Record<string, unknown>
    const employeeName = text(input.employeeName, 160)
    const date = text(input.date, 10)
    const start = text(input.start, 5)
    const end = text(input.end, 5)
    const workArea = text(input.workArea, 160)
    const pauseMinutes = input.pauseMinutes == null || input.pauseMinutes === '' ? 0 : Number(input.pauseMinutes)
  ...
  return {
    ...
    from,
    to,
    responseKey: text(row.responseKey),
    protectedEmployees: protectedRows,
    shifts: cleanedShifts,
  }
}

async function callAssistant(body: Record<string, unknown>, assistantToken: string, context: unknown) {
...
function protectedMatch(name: unknown, protectedEmployees: ReconcileCommand['protectedEmployees']) {
...
function shiftKey(row: Record<string, unknown>) {
  return [
...
async function deleteRows(rows: Array<Record<string, unknown>>, commandId: string) {
...
async function restoreRows(rows: Array<Record<string, unknown>>) {
  for (const row of rows) await upsertScheduleShift(row as never)
}

export default async function scheduleReconcileOidc(request: Request, context: unknown) {
  ...
  let rawCommand: Record<string, unknown>
  try {
  ...
  const encrypted = (payload: unknown) => encryptResult(payload, command.responseKey)
  ...
  const resolutions = Array.isArray(resolved.data.results) ? resolved.data.results as Array<Record<string, unknown>> : []
  const unresolved = resolutions.filter((entry) => String(entry?.status || '') !== 'matched')
  const directoryDiagnostics = resolved.data.directoryDiagnostics && typeof resolved.data.directoryDiagnostics === 'object'
    ? resolved.data.directoryDiagnostics as Record<string, unknown>
    : {}
  if (unresolved.length) {
    return json({
      employeeCount: Number(directoryDiagnostics.employeeCount || 0),
  ...
  try {
    ...
    for (let offset = 0; offset < command.shifts.length; offset += 100) {
      ...
      const results = Array.isArray(published.data.results) ? published.data.results as Array<Record<string, unknown>> : []
  ...
  const expectedKeys = command.shifts.map((row) => shiftKey(row)).sort()
  const actualKeys = actualNonProtected.map((row) => shiftKey(row)).sort()
